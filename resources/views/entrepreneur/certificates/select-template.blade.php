...
<div class="container-fluid py-3 py-md-4">
                ...
                <ul class="nav nav-tabs template-tabs flex-nowrap overflow-x-auto" id="templateCategories" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="category-{{ $category->id }}-tab" data-bs-toggle="tab" 
                                ...
                                aria-controls="category-{{ $category->id }}" aria-selected="false">{{ $category->name }}</button>
                    </li>
                    @endforeach
                </ul>
            </div>
    ...
    <div class="tab-content">
                    ...
                    <div class="col-12 col-sm-6 col-lg-4">
                        <div class="card template-card shadow-sm h-100">
                            <div class="card-img-wrapper">
                                @if ($template->image)
                                    <img src="{{ asset('storage/' . $template->image) }}" class="card-img-top" alt="{{ $template->name }}" loading="lazy">
                                @else
                            ...
                            <div class="card-body p-2 p-sm-3 d-flex flex-column">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <h5 class="card-title fw-bold mb-0 fs-6">{{ $template->name }}</h5>
                            ...
                            <div class="modal-dialog modal-lg modal-dialog-centered modal-fullscreen-md-down">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title fs-6">Предпросмотр "{{ $template->name }}"</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body p-0 p-sm-2">
                                        <div class="preview-container border rounded p-0">
                                            <iframe src="{{ route('template.preview', $template) }}" class="template-preview-iframe" frameborder="0" width="100%" loading="lazy"></iframe>
                                        </div>
                                    </div>
        ...
        <div class="tab-pane fade" id="category-{{ $category->id }}" role="tabpanel" aria-labelledby="category-{{ $category->id }}-tab">
            <div class="row g-2 g-md-3 g-lg-4">
                @php $templatesInCategory = $templates->where('category_id', $category->id); @endphp
                @forelse ($templatesInCategory as $template)
                    <div class="col-12 col-sm-6 col-lg-4">
                        <div class="card template-card shadow-sm h-100">
                            <div class="card-img-wrapper">
                                @if ($template->image)
                            ...
                            <div class="card-body p-2 p-sm-3 d-flex flex-column">
                                <h5 class="card-title fw-bold fs-6">{{ $template->name }}</h5>
                                <p class="card-text text-muted flex-grow-1 small">{{ Str::limit($template->description, 80) }}</p>
                                <a href="{{ route('entrepreneur.certificates.create', $template) }}" class="btn btn-primary btn-sm">
                                    <i class="fa-solid fa-plus me-1 me-sm-2"></i>Выбрать этот шаблон
                                </a>
